...
LOGGER = logging.getLogger("tests.examples.test_linear_random_pytorch_example")
...
def main():
    ...
    parser.add_argument(
        "--protocol", choices=["grpc", "http"], required=False, default="http", help="Which protocol to use by client"
    )
    ...
    verify_docker_image_in_readme_same_as_tested("examples/linear_random_pytorch/README.md", docker_image_with_name)

    start_time = time.time()
    elapsed_s = 0
    wait_time_s = min(args.timeout_s, 1)

    server_cmd = ["python", "examples/linear_random_pytorch/server.py"]
    client_cmd = ["python", "examples/linear_random_pytorch/client.py"]
