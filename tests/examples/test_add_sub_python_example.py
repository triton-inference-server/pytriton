...
LOGGER = logging.getLogger("tests.examples.test_add_python_example")
...
def main():
    ...
    verify_docker_image_in_readme_same_as_tested("examples/add_sub_python/README.md", docker_image_with_name)

    subprocess.run(["bash", "examples/add_sub_python/install.sh"])

    start_time = time.time()
    elapsed_s = 0
    wait_time_s = min(args.timeout_s, 1)

    server_cmd = ["python", "examples/add_sub_python/server.py"]
    client_cmd = ["python", "examples/add_sub_python/client.py"]
