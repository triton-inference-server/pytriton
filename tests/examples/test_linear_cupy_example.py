...
from tests.utils import ScriptThread  # pytype: disable=import-error

LOGGER = logging.getLogger("tests.examples.test_linear_cupy_example")
...
def verify_client_output(client_output):
    output1_match = (
        "result: [[3.0, 4.0, 5.0, 6.0, 7.0, 8.0, 9.0, 10.0, 11.0, 12.0], [3.0, 4.0, 5.0, 6.0, 7.0, 8.0, "
        "9.0, 10.0, 11.0, 12.0]]" in client_output
    )
...
def main():
    ...
    server_cmd = ["python", "examples/linear_cupy/server.py"]
    client_cmd = ["python", "examples/linear_cupy/client.py"]
