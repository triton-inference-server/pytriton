...
def main():
    ...
    with Triton(config=triton_config) as triton:
        ...
        with ModelClient(url, model_spec.name, init_timeout_s=args.init_timeout_s) as client:
            ...
            batch_size = 128
            a_batch = np.ones((batch_size, 32), dtype=np.float32)
            b_batch = np.ones((batch_size, 32), dtype=np.float32)
            ...
            batch_size = 1
            a_batch = np.ones((batch_size, 64), dtype=np.float32)
            b_batch = np.ones((batch_size, 64), dtype=np.float32)
