...
"""Test of multi_instance_linear_pytorch example"""
import argparse
import logging
import re
import signal
...
def verify_client_output(client_output):
    expected_patterns = [
        ...
        r"Pass \[1\] throughput: \d+.\d+ infer/sec\. Avg latency: \d+ usec \(std \d+ usec\)",
        r"Pass \[2\] throughput: \d+.\d+ infer/sec\. Avg latency: \d+ usec \(std \d+ usec\)",
        r"Pass \[3\] throughput: \d+.\d+ infer/sec\. Avg latency: \d+ usec \(std \d+ usec\)",
    ]
...
def main():
    ...
    verify_docker_image_in_readme_same_as_tested(
        "examples/multi_instance_resnet50_pytorch/README.md", docker_image_with_name
    )

    install_cmd = ["bash", "examples/multi_instance_resnet50_pytorch/install.sh"]
    ...
    server_cmd = ["python", "examples/multi_instance_resnet50_pytorch/server.py"]
    client_cmd = ["bash", "examples/multi_instance_resnet50_pytorch/client.sh"]

    with ScriptThread(server_cmd, name="server") as server_thread:
        ModelClient("localhost", "ResNet50").wait_for_model(timeout_s=args.timeout_s)
