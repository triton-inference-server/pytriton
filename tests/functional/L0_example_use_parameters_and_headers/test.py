...
"""Test of use_parameters_and_headers example"""
import argparse
import logging
import signal
import subprocess
...
def verify_client_output(client_output):
    output1_match = "scaled_add: [[1.0], [1.0]]" in client_output
    output2_match = "scaled_sub: [[2.0], [2.0]]" in client_output
...
def main():
    ...
    verify_docker_image_in_readme_same_as_tested(
        "examples/use_parameters_and_headers/README.md", docker_image_with_name
    )

    subprocess.run(["bash", "examples/use_parameters_and_headers/install.sh"])

    start_time = time.time()
    elapsed_s = 0
    wait_time_s = min(args.timeout_s, 1)

    server_cmd = ["python", "examples/use_parameters_and_headers/server.py"]
    client_cmd = ["python", "examples/use_parameters_and_headers/client.py"]
