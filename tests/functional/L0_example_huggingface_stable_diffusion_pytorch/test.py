...
def verify_client_output(results_path):
    file_path = results_path / "1" / "image.jpeg"
    if not file_path.exists():
        raise ValueError(f"Unable to find the generated image at {file_path}.")
...
def main():
    ...
    verify_docker_image_in_readme_same_as_tested(
        "examples/huggingface_stable_diffusion//README.md", docker_image_with_name
    )

    install_cmd = ["bash", "examples/huggingface_stable_diffusion/install.sh"]
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        results_path = pathlib.Path(tempdir)
        server_cmd = ["python", "examples/huggingface_stable_diffusion/server.py"]
        client_cmd = [
            "python",
            "examples/huggingface_stable_diffusion/client.py",
            "--results-path",
        ...
        if server_thread.returncode not in [
            0,
            -2,
        ]:  # -2 is returned when process finished after receiving SIGINT signal
        ...
        if timeout:
            ...
            sys.exit(-2)
        verify_client_output(results_path)
