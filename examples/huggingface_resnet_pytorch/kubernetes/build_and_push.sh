...
if [ -z ${FROM_IMAGE_NAME} ]; then
  ...
  echo "Example:"
  echo "    export FROM_IMAGE_NAME=nvcr.io/nvidia/pytorch:24.09-py3"
  exit 1
fi
...
DOCKER_BUILDKIT=1 docker build -f examples/huggingface_resnet_pytorch/kubernetes/Dockerfile \
