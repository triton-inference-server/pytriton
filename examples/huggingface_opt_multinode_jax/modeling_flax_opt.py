...
class FlaxOPTAttention(nn.Module):
    ...
    def setup(self) -> None:
        ...
        dense = partial(
            ...
            kernel_init=jax.nn.initializers.normal(self.config.init_std, dtype=self.dtype),
        )
...
class FlaxOPTDecoderLayer(nn.Module):
    ...
    def setup(self) -> None:
        ...
        self.final_layer_norm = nn.LayerNorm(dtype=self.dtype, epsilon=1e-05, param_dtype=self.dtype)
...
class FlaxOPTForCausalLM(FlaxOPTPreTrainedModel):
    ...
    def update_inputs_for_generation(self, past_key_values, model_kwargs):
