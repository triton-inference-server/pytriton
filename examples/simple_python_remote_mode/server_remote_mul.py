...
logger = logging.getLogger("examples.simple_python_remote_mode.server_remote_mul")
...
def _mul(**inputs):
    ...
    dot_result = np.multiply(a_batch, b_batch)
...
with RemoteTriton(url="localhost") as remote_triton:
    remote_triton.bind(
        ...
        outputs=[
            Tensor(name="mul", dtype=np.float32, shape=(-1,)),
        ],
