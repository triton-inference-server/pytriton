...
2. Install requirements required by example and [PyTriton](../../docs/installation.md):
    ```bash
    docker exec ft_on_pytriton bash -c "./examples/fastertransformer_gpt_multinode/install.sh && pip install -U nvidia-pytriton"
    ```
...
1. Prepare the slurm submission file. Example Slurm `sbatch` submission file:
   ...
   srun --output slurm_job-%x-%J.out \
        ...
        --unbuffered \
        bash -c '[[ ${LOCAL_RANK} -eq 0 ]] && (./examples/fastertransformer_gpt_multinode/install.sh && pip install -U nvidia-pytriton) || true'
